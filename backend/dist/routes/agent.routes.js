...
router.post('/goal', async (req, res) => {
    const { goal, groqApiKey, userId } = req.body;
    ...
    try {
        const result = await (0, pipeline_1.runPipeline)(userId, goal.trim(), apiKey, (event) => {
...
router.get('/status/:userId', async (req, res) => {
    ...
    const stats = await (0, pipeline_1.getStats)(userId);
    res.status(200).json({
        success: true,
        status: 'operational',
        stats
...
router.get('/memory/:userId', async (req, res) => {
    ...
    const memory = await (0, pipeline_1.getMemoryStore)(userId);
    res.status(200).json({
        success: true,
        count: memory.length,
        data: memory
